...
import Plymouth from '../Assets/Counties/Plymouth';
import Middlesex from '../Assets/Counties/Middlesex';
...
function AddProperty() {
	...
	function ReducerFuction(draft, action) {
		switch (action.type) {
			...
			case "catchCountyChange":
				...
				draft.countyErrors.hasErrors = false;
				draft.countyErrors.errorMessage = "";
				break;
			...
			case "catchCountyErrors":
				if (action.countyChosen.length === 0) {
					draft.countyErrors.hasErrors = true;
					draft.countyErrors.errorMessage = "This field must not be empty";
				}
				break;
    ...
    // Use effect to change the map view depending on chosen county
	// Changing the map view depending on the chosen county

	useEffect(() => {
		if (state.countyValue === "Suffolk") {
			state.mapInstance.setView([42.36033157198816, -71.05448146601047], 12);
			dispatch({
				type: "changeMarkerPosition",
				changeLatitude: 42.36033157198816,
				changeLongitude: -71.05448146601047,
			});
		} else if (state.countyValue === "Norfolk") {
			state.mapInstance.setView([42.33997156101429,-71.11956124516858], 12);
			dispatch({
				type: "changeMarkerPosition",
				changeLatitude: 42.33997156101429,
				changeLongitude: -71.11956124516858,
			});
		} else if (state.countyValue === "Plymouth") {
            state.mapInstance.setView([42.089771512770305, -70.98686033624077], 12);
			dispatch({
				type: "changeMarkerPosition",
				changeLatitude: 42.089771512770305,
				changeLongitude: -70.98686033624077,
			});
        } else if (state.countyValue === "Middlesex") {
            state.mapInstance.setView([42.510719889096436, -71.16753612318684], 12);
			dispatch({
				type: "changeMarkerPosition",
				changeLatitude: 42.510719889096436,
				changeLongitude: -71.16753612318684,
			});
        }
	}, [state.countyValue]);
	...
	function countyDisplay() {
		if (state.countyValue === "Suffolk") {
            ...
            return <Polygon positions={Plymouth} />;
        } else if (state.countyValue === "Middlesex") {
            return <Polygon positions={Middlesex} />;
        } 
	}
	...
	function FormSubmit(e) {
		...
		if (
			...
			window.scrollTo(0, 0);
		} else if (state.countyValue === "") {
			dispatch({ type: "emptyCounty" });
			window.scrollTo(0, 0);
		}
	}
				...
				<Grid item container justifyContent="space-between">
					...
					<Grid item xs={5} style={{ marginTop: "1rem" }}>
						<TextField
							...
							onBlur={(e) =>
								dispatch({
									type: "catchCountyErrors",
									countyChosen: e.target.value,
								})
							}
							error={state.countyErrors.hasErrors ? true : false}
							helperText={state.countyErrors.errorMessage}
							select
							SelectProps={{
								native: true,
							}}
